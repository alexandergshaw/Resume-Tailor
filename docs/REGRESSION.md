...
**Known coverage gap:** the hook's wiring of that decision into `onUtterance` is glue reachable only by mounting a hook, which this repo cannot do. Making the detector ignore the rule entirely turns nothing red. The DECISION is pure and tested; its application is verified by reading.
...
**Known coverage gap, stated rather than papered over.** The composition inside `useCopilotDashboard` — that the hook calls `speculativeWorkEnabled` with the right arguments and uses the result at every gating site — is NOT falsifiable by any test here: `vitest.config.js` is `environment: "node"` with no jsdom, so the hook body cannot be executed. Inverting that one line leaves the suite green, and the implementing agent reported this honestly rather than claiming coverage. What IS tested is both halves it composes: `speculativeWorkEnabled`'s truth table, and `resolveDashboardState` forcing the idle state whenever its gate is false (mutating that one turns three tests red). Step 4 is the only real check on the wiring, which is why it is a network observation and not a visual one.
