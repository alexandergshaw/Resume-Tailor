...
**Steps:**
1. Read `readChatResponse` in `hello-world/lib/chat/chatbot.js:146-202` and confirm the only body read is the single `await response.text()` at `:152`, that `JSON.parse` runs on that string in its own try/catch at `:161-165`, and that no branch below it ever calls `.json()` — the range ends at the function's own closing brace, so it includes the status-413 branch at `:197-199` that this case exists to check. *(Anchors moved by the PLAN-A1 applications-context extraction, which added one import line and several comment blocks above this code; content unchanged. Re-verified line by line in the manual pass: the previously cited `:136-192` was short by ten lines, as was every other `chatbot.js` anchor in R-284 through R-295.)*
...
**Steps:**
1. Read the status gate in `readChatResponse` (`hello-world/lib/chat/chatbot.js:187`) and confirm it explicitly excludes status 500 (`status !== 500 && parsed && typeof parsed.error === "string" && parsed.error`) before any verbatim pass-through.
2. Read `hello-world/app/api/chat/route.js:258` and confirm the catch-all still returns `Response.json({error: err?.message || "Chat request failed."}, {status: 500})` — the branch in step 1 exists specifically to keep that string off the screen. *(Anchors moved by the PLAN-A1 applications-context extraction; content unchanged.)*
...
**Steps:**
1. Read the constants block in `hello-world/lib/chat/chatbot.js:9-80`: `MAX_REQUEST_BYTES` (`:44`, 4,500,000), `CHAT_BODY_OVERHEAD_BYTES` (`:48`, 500,000), `MAX_ATTACHMENT_PAYLOAD_BYTES` (`:50`, their difference, 4,000,000), `MAX_BINARY_ATTACHMENT_BYTES` (`:56`, 3,000,000), the `"2.8 MB"` label (`:61`), and `base64Length` (`:78-80`). *(Anchor moved by the PLAN-A1 applications-context extraction; content unchanged. The range is EXTENDED, not merely shifted: the previously cited `:9-70` stopped short of `base64Length` — the constants themselves end at `:72` and `base64Length` sits at `:78-80`, outside any ten-line shift of the old range — so a reader following the old anchor could not see the one function this case's Expected is mostly about.)*
2. From `hello-world`, run `npx vitest run lib/chat/chatbot.response.test.js`.
...
**Steps:**
1. Read `attachmentCost` in `hello-world/lib/chat/chatbot.js:321-330` and confirm the text/`.docx` branch encodes `entry.content` with `TextEncoder` (UTF-8 byte length), not `.length` (UTF-16 code units) — the binary branch's `dataB64` is pure ASCII base64 and needs no such adjustment.
2. Read the `.docx` branch of `addChatAttachments` (`hello-world/lib/chat/chatbot.js:372-375`) and confirm the `MAX_DOCX_SOURCE_BYTES` (25 * 1024 * 1024) check runs on `file.size` BEFORE `buildTemplateLinesForUpload` (`:380`) — and therefore before `JSZip.loadAsync` — is ever called, and is a separate check from the per-file `MAX_BINARY_ATTACHMENT_BYTES` gate, which is explicitly skipped for `.docx` (`:363`, `!isDocx && file.size > MAX_BINARY_ATTACHMENT_BYTES`). *(Anchors moved by the PLAN-A1 applications-context extraction; content unchanged.)*
...
**Steps:**
1. Read `addChatAttachments` in `hello-world/lib/chat/chatbot.js:332-432` and confirm refusals accumulate onto an `errors` array (declared at `:339`) across the whole loop and are joined once, at `:427` (`setChatAttachError(errors.join(" "))`), rather than overwriting a single error slot per file. *(Anchors moved by the PLAN-A1 applications-context extraction; content unchanged.)*
...
**Steps:**
1. Read the success branch of `runChatRequest` in `hello-world/lib/chat/chatbot.js:617-632` and confirm it filters `prev` by reference against the `sentAttachments` snapshot taken before the request went out (`:551`), so anything attached after that snapshot but before the response resolves is left in the tray untouched.
2. Confirm the catch branch (`:634-649`) never reads or writes `chatAttachedFiles` at all. *(Anchors moved by the PLAN-A1 applications-context extraction, which inserted the client-side projection call at `:541`, immediately ABOVE the snapshot — not between the snapshot and the request body. The snapshot is still the last statement before `JSON.stringify` builds the body at `:554`, which is the ordering this case exists to pin; content unchanged.)*
...
**Steps:**
1. Read the catch branch of `runChatRequest` (`hello-world/lib/chat/chatbot.js:634-649`) and confirm it marks the last user turn `failed: true` in place (`:645`) rather than appending a new one; read the top of `runChatRequest` (`:469-473`) and confirm a trailing `failed` user turn already present in `baseMessages` is popped before the new user turn is appended, so a retry reuses that slot instead of stacking a second, larger copy of the same turn. *(Anchors moved by the PLAN-A1 applications-context extraction; content unchanged.)*
...
**Steps:**
1. Read `revokeAttachmentPreview` in `hello-world/lib/chat/chatbot.js:87-95` (guarded on `typeof URL`, not just try/catch, so it degrades to a no-op rather than throwing when the `URL` global is absent, as in a node test environment) and its three call sites: the success branch of `runChatRequest` (`:626`), the Clear handler in `hello-world/app/components/ChatPanel.js:172` (`chatAttachedFiles.forEach(revokeAttachmentPreview)`), and the per-chip `onDelete` handler (`:392-397`). *(chatbot.js anchors moved by the PLAN-A1 applications-context extraction; ChatPanel.js anchors untouched by that change.)*
...
3. **There is no attachment or combination that passes the client's gates and still reaches the platform's cap — the step this case used to ask for does not exist.** Three gates stack, all below Vercel's 4.5 MB: the per-file gate refuses a non-`.docx` above 3,000,000 bytes (`chatbot.js:363`); the aggregate gate refuses once the running attachment payload would exceed `MAX_ATTACHMENT_PAYLOAD_BYTES` = 4,000,000 (`:405`); and `runChatRequest` measures the whole serialized body and throws above `MAX_REQUEST_BYTES` = 4,500,000 **before `fetch` is ever called** (`:588-591`). Provoking a genuine platform 413 therefore requires **bypassing the client entirely** — issuing a direct `POST` to the deployed `/api/chat` endpoint with a body over 4.5 MB (curl, Postman, or a devtools console `fetch`), which is what this step must say and previously did not. Do that, then open the response.
...
The previously stated "order 10⁴ … measured 26,086" is the TEST-fixture row only, and it is the least representative of the three. At that scale the message still names saved application history as the cause (`TOO_BIG_NO_ATTACHMENTS_MESSAGE`, `hello-world/lib/chat/chatbot.js:131-135`) — NOT "remove an attachment" (`TOO_BIG_MESSAGE`, `:120-122`), which remains impossible advice with an empty tray. *(The earlier anchor `:121-125` for `TOO_BIG_NO_ATTACHMENTS_MESSAGE` landed on `TOO_BIG_MESSAGE` — the exact wording this case says must NOT appear.)*
