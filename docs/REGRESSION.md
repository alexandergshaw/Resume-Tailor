...
**Expected:** Every automated case passes and step 5 shows an empty, service-role-only `drive_connections`. No user-visible behaviour changes anywhere in the app, before or after — if anything Drive-shaped appears in the UI after this push, something was mounted that this wave did not intend to mount.
...
**One live oddity that is safe only by accident of scope.** `"email"` remains a deliberate fixture value in `driveSaveBatch.test.js` — a real `SCOPES` member but NOT a `DOCX_SCOPES` one, and the migration's CHECK rejects it. It is safe today only because `driveSaveBatch` is pure decision logic that never touches the database, and the fixture only exercises row ordering. The moment any code path carries a `ScopeOutcome.scope` into `upsertDriveDocument`, that fixture becomes a lie about what the schema accepts.
...
7. Manual (`automatable: no`): confirm there is nothing to confirm. No route, no mount, no reachable UI.

**Expected:** Every automated case passes, step 6 finds no unresolved path and no bad binding name, and the running app is byte-for-byte the same experience before and after this push. If anything Drive-shaped appears in the UI, something was mounted that this wave did not intend to mount.
...
8. Manual (`automatable: no`): confirm there is nothing to confirm. Seven routes exist and answer, and no page in the app calls any of them. `grep -rn "/api/drive" hello-world/app hello-world/lib --include=*.js` outside `app/api/drive/` and its tests should find only `DriveButton.js`, which is itself mounted nowhere.

**Expected:** Every automated case passes, step 8 finds no mount, and the running app is byte-for-byte the same experience before and after this push. If anything Drive-shaped appears in the UI, something was mounted that this wave did not intend to mount.
...
4. Read `performSave` in `hello-world/app/hooks/useDriveDocuments.js:727-744` and confirm the per-scope `await attemptOneScope(scope, args)` is wrapped in its own `try/catch` that pushes an `errorKind: "unknown"` outcome and `continue`s — a `finally` alone is not the guarantee. Then confirm the currency-hash effect at `:451` has a `.catch`, matching its neighbour at `:400-411`. Pinned by `useDriveDocuments.connect.test.js:646` and `:673`.
5. Read `hello-world/app/components/DocumentPreviewMount.js:22-34` and confirm `currentUser`, `resumeFile` and `coverLetterFile` are still in the signature with the comment saying why they are accepted but unused, and that `app/page.js:3170` still passes all three.
...
7. Manual (`automatable: no`): open the app, sign in, open the settings gear, and confirm a **Google Drive** section with a `Connect Drive` button sits between Gmail and Account. Then open the tailored-documents preview modal and confirm there is still **no** Drive save or download control anywhere in it. Both halves matter: the first is the new user-reachable surface, the second is the boundary this wave deliberately did not cross.
...
**Expected:** Every automated case passes, step 7 finds the settings section and finds no Drive control in the preview modal, and step 8's console is clean.
...
**Expected:** Every automated case passes, the owner setup in step 9 is complete, step 10 connects and returns to a signed-in session, and step 11 round-trips both documents. A failure at step 10 with steps 1-8 green means the cookie survived nothing and the fault is in deployment or in a `@supabase/ssr` upgrade, not in this wave's code.
