...
//
// ORDERING: none of the three tables below re-sorts its items — each is `.filter(...)` only, so
// the row order is exactly docs/backlog.yml's own array order (the owner's hand-set priority, in
// ALL THREE sections per R-BL-1, backlog N31). Reintroducing `.sort(compareIds...)` at any of the
// three sites below reproduces that defect; see orderClassGuard.test.js for the regression sweep.
...
export function renderMarkdown(items) {
  const n = items.filter((it) => it.state === "actionable");
  const d = items.filter((it) => it.state === "owner");
  const v = items.filter((it) => it.state === "verification");
