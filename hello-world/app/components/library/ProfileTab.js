...
export default function ProfileTab({ profile, onChanged }) {
  ...
  return (
    // TOUCH_FIELD_SX is a descendant-selector fragment, so one spread on the
    // tab root floors every value field, the "New key" field and the chips
    // input at once -- including a field added here later.
    <Box sx={TOUCH_FIELD_SX}>
      ...
      <Stack spacing={1}>
        {Object.keys(values).sort().map((k) => (
          <Stack direction="row" spacing={1} key={k} sx={{ alignItems: "center" }}>
            ...
            <IconButton aria-label="remove" sx={TOUCH_ICON_SX} onClick={() => setValues((v) => { const n = { ...v }; delete n[k]; return n; })}><DeleteIcon fontSize="small" /></IconButton>
          </Stack>
        ))}
      </Stack>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ mt: 2, alignItems: { xs: "stretch", sm: "center" } }}>
        ...
        <Button sx={TOUCH_TARGET_SX} onClick={() => { if (newKey && !(newKey in values)) { setValues((v) => ({ ...v, [newKey]: "" })); setNewKey(""); } }}>Add key</Button>
      </Stack>
      ...
      <Button variant="contained" size="small" disableElevation sx={{ mt: 3, textTransform: "none", borderRadius: 1.5, ...TOUCH_TARGET_SX }} onClick={save} disabled={saving}>{saving ? "Saving…" : "Save profile"}</Button>
    </Box>
  );
}
