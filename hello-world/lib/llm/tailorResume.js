...
function buildTailorPrompt({
  ...
  return [
    ...
    "13) The skills section is in scope for rewriting at every aggressiveness level. Update the listed skills so they directly mirror the tools, technologies, languages, frameworks, methodologies, and domain terms named in the posting, using the posting's exact spelling and casing. Reorder skills so the ones most emphasized by the posting appear first within their category. You may drop source-resume skills that are clearly irrelevant to the posting to make room for relevant ones, and you may add posting skills so long as they will make the resume a better match for the posting. Preserve the section's existing structure (category labels, separators, line count, and slot order) — only the category labels and skill tokens themselves change.",
