...
// Joins two whole-page entries inside the block. Still exported even though
// nothing imports it today: its sole importer was a test that needed the
// literal to prove a forged "---" was defused, and that suite was deleted
// along with the unreachable module it covered. The export is retained
// deliberately (it is carried in
// lib/sourceScan/exportReachability.sweep.test.js's reviewed-and-kept
// ledger) so the NEXT test that needs this literal imports
// the constant instead of hardcoding "---" a second time — a hardcoded copy
// would keep passing even if this separator's shape ever changed, silently
// going stale exactly like the anti-pattern flagged for meetingContext.js.
