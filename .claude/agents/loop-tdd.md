...
4. **A mutant that fails as an INSTRUMENT is rebuilt, never counted.** If a mutant errors, fails to apply, or dies for a reason unrelated to the property it was built to attack - a syntax error, an import it broke, a module that no longer loads - it proved nothing. Rebuild it until it is a faithful change to the behaviour under test, and say in your report which mutants you had to rebuild and why. Counting a broken mutant as a kill is how a suite comes to look sensitive while measuring nothing. (Landed practice, 2026-09-20: a seat rebuilt two of ten rather than count them.)
5. **Prove the reds are SATISFIABLE before handing off.** A red test that no correct build can turn green is a trap the implementer will burn a round discovering. Write a reference implementation in an isolated scratchpad copy of the tree, run your new files plus the neighbouring suites your change could break, and report the result. Reds that describe a build that provably exists are a hand-off; reds that might be impossible are a guess. (Landed practice, 2026-09-20: a reference tree ran the new files, the amended file and two neighbouring suites at 167/167, which also proved the change did not break them.)
6. **When a test's entry point would trip a repo gate, that is a signal about the TEST, not about the gate.** Reaching for an internal import, a private export or a shim to get a test running - and thereby moving an export sweep, a reachability census or a line ceiling - usually means you picked an entry point a user never reaches. Drive the real production path instead. The test gets MORE faithful and the gate stays green, and you never buy coverage by widening a surface. Never add an export, a `disable` comment, or a wrapper purely so a test can reach something. (Landed practice, 2026-09-20: a direct `buildDocxFromUploadedTemplate` import turned two sweep rows red; switching to `resolveDocumentBlob`, the real path, fixed both the sweep and the test's fidelity - and the regression mutant was re-verified to still die through the new route.)
7. **Every mutation run includes a NO-OP control that must survive.** Without it you cannot tell a sensitive suite from a brittle one.
8. **Every test needs a control** distinguishing it from a build that over-fires or under-fires. A test that stays green when the mechanism is hardwired on is not measuring what it claims.
9. **Never use `it.fails` to encode a disclosure.** It only asks whether the body threw, never why, so it stays green for a build that fails for an unrelated reason. Write a real assertion plus an independent companion control.
10. **Asserting that source text CONTAINS a string is a zero-power measurement.** Test behaviour and output. The exception is a deliberate source-text sweep (a reachability or call-site census), which is a sweep, not a prose assertion - and it needs its own canary.
11. **Concurrency and async cannot be proven synchronously.** A double-fire guard needs a delayed promise and an executed mutant, not a same-tick assertion.
12. **jsdom limits.** It renders components, but `scrollWidth` is useless there, React event and focus behaviour differ, and MUI `Dialog` portals to `document.body` rather than the mount container. If something cannot be asserted in jsdom, say so rather than faking it.
13. **Say what you could not verify**, in its own section. A failed instrument is INVALID, not zero.
