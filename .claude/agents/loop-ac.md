...
6b. **When the chunk adds to a surface other chunks also add to, say what it ADDS and what it DISPLACES.** Gates are per-diff; a surface's legibility is cumulative and no gate measures it. One panel here went from creation to 671 lines across six chunks in a single day, each passing every gate it had, before the owner reported it unreadable. You are not being asked to redesign the surface - only to make the composite cost visible at the point the work is specified, so the 8b re-judge has something to weigh.
