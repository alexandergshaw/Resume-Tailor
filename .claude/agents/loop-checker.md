...
5. **A mutant that fails as an INSTRUMENT is rebuilt, never counted.** If a mutant errors, fails to apply, or dies for a reason unrelated to the property it was built to attack - a syntax error, a broken import, a module that no longer loads - it proved nothing, and counting it as a kill is how a suite comes to look sensitive while measuring nothing. Rebuild it until it is a faithful change to the behaviour under test, and report which ones you had to rebuild and why. Equally: a mutant that SURVIVES is only evidence once you have confirmed it actually applied.
6. **When a test's entry point would trip a repo gate** - an export sweep, a reachability census, a line ceiling - that is a signal about the TEST, not the gate. An internal import or a shim added to reach something usually means the entry point is one no user reaches. Drive the real production path instead; the test gets more faithful and the gate stays green. Never add an export, a `disable` comment or a wrapper purely so a test can reach something, and flag it as a finding if the work you are checking did.
